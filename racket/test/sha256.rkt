...
(require "generated/racket/test/sha256.ll.rkt")


(define (check-test-vector hex-data hex-expected)
  (define data (hex-string->bytes hex-data))
  (define len (bytes-length data))
  (define expected (map core:bv8 (bytes->list (hex-string->bytes hex-expected))))

  ; Skip testing the functional spec as that's too slow.
  ; (let-values ([(actual asserted)
  ;               (with-asserts (sha256-hash (map core:bv8 (bytes->list data))))])
  ;   (check-equal? asserted null)
  ;   (check-equal? actual expected))

  ; test the C implementation
  (parameterize ([llvm:current-machine (llvm:make-machine)])
    (define input (core:marray len (core:mcell 1)))
    ...
    (define asserted
      (with-asserts-only
        (@sha256_hash (core:make-pointer output)
                      (core:make-pointer input)
                      (core:bvpointer len))))
    (check-equal? asserted null)
    ; extract output
    (define actual (for/list ([i (in-range 32)])
...
(define sha256-state-size 137)

; the last 9 bytes of Hacl SHA256 store the state and count
(define (abs-function ctx)
  (define state
    (for/list ([i (in-range (- sha256-state-size 9) (sub1 sha256-state-size))])
      (core:mblock-iload ctx (list (core:bvpointer i)))))
  (define count (core:mblock-iload ctx (list (core:bvpointer (sub1 sha256-state-size)))))
  (sha256 state count))

(define (rep-invariant ctx)
  (define preds
    (for/list ([i (in-range (vector-length K))])
      (equal? (bv (vector-ref K i) 32) (core:mblock-iload ctx (list (core:bvpointer i))))))
  (apply && preds))

(define (verify-refinement-sha256_init)
  (parameterize ([llvm:current-machine (llvm:make-machine)])
    (define asserted
      (with-asserts-only (begin
        (define ctx (core:marray sha256-state-size (core:mcell 4)))
        (@sha256_init (core:make-pointer ctx))
        (check-true (rep-invariant ctx))
        (check-equal? (abs-function ctx) (sha256-init)))))
    (check-equal? asserted null)))


(define (make-bv8)
  (define-symbolic* symbolic-byte (bitvector 8))
  symbolic-byte)

(define (make-data n)
  (build-list n (lambda (x) (make-bv8))))

(define (make-bv32)
  (define-symbolic* symbolic-word (bitvector 32))
  symbolic-word)

(define (make-state)
  (build-list 8 (lambda (x) (make-bv32))))

(define (verify-refinement-sha256_update)
  (parameterize ([llvm:current-machine (llvm:make-machine)])
    (define asserted
      (with-asserts-only (begin
        (define state (make-state))
        (define-symbolic count (bitvector 32))
        (define impl-state (core:marray sha256-state-size (core:mcell 4)))
        (define data (make-data 64))
        (define input (core:marray 64 (core:mcell 1)))
        (define spec-state (sha256 state count))

        (for ([i (in-range (vector-length K))])
          (core:mblock-istore! impl-state (bv (vector-ref K i) 32) (list (bv i 64))))
        (for ([i (in-range (length state))])
          (core:mblock-istore! impl-state (list-ref state i) (list (bv (+ (- sha256-state-size 9) i) 64))))
        (core:mblock-istore! impl-state count (list (bv (sub1 sha256-state-size) 64)))

        (for ([i (in-range 64)])
          (core:mblock-istore! input (list-ref data i) (list (bv i 64))))

        (define ri-pre (rep-invariant impl-state))
        (check-true ri-pre)

        (@sha256_update (core:make-pointer impl-state) (core:make-pointer input))

        (define ri-post (rep-invariant impl-state))
        (check-true ri-post)

        (define ctx1 (abs-function impl-state))
        (define ctx2 (sha256-update spec-state data))
        ; (check-unsat? (verify (assert (equal? (sha256-count ctx1) (sha256-count ctx2)))))
        (check-unsat? (verify (assert (equal? (list-ref (sha256-state ctx1) 0)
                                              (list-ref (sha256-state ctx2) 0)))))
        ;(define sol (verify (assert (equal? ctx1 ctx2))))
        ; (when (sat? sol)
        ;   (displayln sol))
      )))
        ;(check-unsat? (verify (assert (equal? (abs-function impl-state) (sha256-update spec-state data))))))))

    (check-equal? asserted null)))

(define-syntax-rule (test-vector name data result)
  (test-case+ name (check-test-vector data result)))

(define sha256-tests
  (test-suite+ "Tests for SHA-256"
    ; refinement
    ; (test-case+ "refinement: sha256_init"
    ;   (verify-refinement-sha256_init))
    ; (test-case+ "refinement: sha256_update"
    ;   (verify-refinement-sha256_update))


    ; FIPS 180-4, short messages
    (test-vector "Len = 0"
      ""
