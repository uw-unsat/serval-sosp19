...
(define enable-split-pc (make-parameter (if (getenv "DISABLE_SPLIT_PC") #f #t)))
...
(define (interpret-program cpu program)
  ...
  (cond
    ...
    [#t
      (define pc (cpu-pc cpu))
      (define instr (for/all ([p pc #:exhaustive]) (if (hash-has-key? instructions p) (hash-ref instructions p) #f)))
      (when (! (equal? 'mret (instr-op instr)))
        (interpret-instr cpu instr)
        (interpret-program cpu program))]))