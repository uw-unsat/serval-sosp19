			      ...
			      const u8 op)
{
	...
	case BPF_LSH:
		if (imm >= 32) {
			emit(rv_slli(rd[0], rd[1], imm - 32), ctx);
			emit(rv_addi(rd[1], RV_REG_ZERO, 0), ctx);
		} else if (imm == 0) {
			/* nop */
		} else {
			emit(rv_srli(RV_REG_T0, rd[1], 32 - imm), ctx);
			emit(rv_slli(rd[0], rd[0], imm), ctx);
			emit(rv_or(rd[0], RV_REG_T0, rd[0]), ctx);
			emit(rv_slli(rd[1], rd[1], imm), ctx);
		}
		break;
	case BPF_RSH:
		if (imm >= 32) {
			emit(rv_srli(rd[1], rd[0], imm - 32), ctx);
			emit(rv_addi(rd[0], RV_REG_ZERO, 0), ctx);
		} else if (imm == 0) {
			/* nop */
		} else {
			emit(rv_slli(RV_REG_T0, rd[0], 32 - imm), ctx);
			emit(rv_srli(rd[1], rd[1], imm), ctx);
			emit(rv_or(rd[1], RV_REG_T0, rd[1]), ctx);
			emit(rv_srli(rd[0], rd[0], imm), ctx);
		}
		break;
	case BPF_ARSH:
		if (imm >= 32) {
			emit(rv_srai(rd[1], rd[0], imm - 32), ctx);
			emit(rv_srai(rd[0], rd[0], 31), ctx);
		} else if (imm == 0) {
			/* nop */
		} else {
			...
			emit(rv_srli(rd[1], rd[1], imm), ctx);
			emit(rv_or(rd[1], RV_REG_T0, rd[1]), ctx);
			emit(rv_srai(rd[0], rd[0], imm), ctx);
		}
		break;
	}
